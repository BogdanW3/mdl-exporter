...
def parse_geoset_animations(data: str) -> War3GeosetAnim:
    ...
    if geoset_id != "Multiple":
        ...
        geoset_animation.geoset_name = geoset_id
    ...
    for info in animation_info:
        ...
        if info.find("Color") > -1:
            if info.find("static Color") > -1:
                geoset_animation.color_anim = make_fake_animation(extract_float_values(get_between(info, "static Color ", "END")))
            else:
                color_chunk = re.split("\\s*(?=Color)", animation_data, 1)[1]
                geoset_animation.color_anim = parse_geoset_transformation(color_chunk)
            # layer.material_alpha = float(get_between(info, "Alpha ", ","))
        ...
        if not geoset_animation.color_anim:
            geoset_animation.color_anim = make_fake_animation([0, 0, 0])
        if not geoset_animation.alpha_anim:
            geoset_animation.alpha_anim = make_fake_animation(1)

    return geoset_animation


def make_fake_animation(values):
    fake_transform = War3AnimationCurve()
