...
class War3Model:
    ...
    def add_particle_systems(self, billboard_lock, billboarded, mats, obj, parent, settings):
        ...
        if getattr(data, "mdl_particle_sys"):
            ...
            register_global_sequence(self.global_seqs, psys.visibility)
    ...
    def is_animated_ugg(self, obj, settings):
        anim_loc = War3AnimationCurve.get(obj.animation_data, 'location', 3,
                                          self.sequences)  # get_curves(obj, 'location', (0, 1, 2))
        if anim_loc is not None and settings.optimize_animation:
            anim_loc.optimize(settings.optimize_tolerance, self.sequences)
        anim_rot = War3AnimationCurve.get(obj.animation_data, 'rotation_quaternion', 4,
                                          self.sequences)  # get_curves(obj, 'rotation_quaternion', (0, 1, 2, 3))
        if anim_rot is None:
        ...
        anim_scale = War3AnimationCurve.get(obj.animation_data, 'scale', 3,
                                            self.sequences)  # get_curves(obj, 'scale', (0, 1, 2))
    ...
    def make_mesh(self, billboard_lock, billboarded, context, mats, obj,
                  parent, settings):
        visibility = get_visibility(self.sequences, obj)
        anim_loc, anim_rot, anim_scale, is_animated = self.is_animated_ugg(obj, settings)
            ...
            if mat is not None and hasattr(mat, "node_tree") and mat.node_tree is not None:
                ...
                if node is not None:
                    ...
                    if hasattr(mat.node_tree, "animation_data"):
                        vertexcolor_anim = War3AnimationCurve.get(mat.node_tree.animation_data,
                                                                  'nodes["VertexColor"].%s[0].default_value' % attr, 3,
                                                                  self.sequences)
        geoset_anim = None
        geoset_anim_hash = 0
        ...
        for tri in mesh.loop_triangles:
            ...
            for vert, loop in zip(tri.vertices, tri.loops):
                ...
                if armature is not None:
                    vgroups = sorted(mesh.vertices[vert].groups[:], key=lambda x: x.weight,
                    ...
                    if len(vgroups):
                        ...
                        groups = list(obj.vertex_groups[vg.group].name for vg in vgroups if
            ...
            geoset.triangles.append(
                (vertexmap[tri.vertices[0]], vertexmap[tri.vertices[1]], vertexmap[tri.vertices[2]]))
    ...
    def add_empties_animations(self, billboard_lock, billboarded, obj, parent, settings):
        ...
        if obj.name.startswith("SND") or obj.name.startswith("UBR") or obj.name.startswith(
            ...
            for datapath in ('["event_track"]', '["eventtrack"]', '["EventTrack"]'):
                eventobj.track = War3AnimationCurve.get(obj.animation_data, datapath, 1,
                                                        self.sequences)  # get_curve(obj, ['["eventtrack"]', '["EventTrack"]', '["event_track"]'])
    ...
    def add_bones(self, billboard_lock, billboarded, obj, parent, settings):
        ...
        for b in obj.pose.bones:
            ...
            bone.anim_loc = War3AnimationCurve.get(obj.animation_data, datapath % 'location', 3,
                                                   self.sequences)  # get_curves(obj, datapath % 'location', (0, 1, 2))
            ...
            bone.anim_rot = War3AnimationCurve.get(obj.animation_data, datapath % 'rotation_quaternion', 4,
                                                   self.sequences)  # get_curves(obj, datapath % 'rotation_quaternion', (0, 1, 2, 3))
            if bone.anim_rot is None:
                bone.anim_rot = War3AnimationCurve.get(obj.animation_data, datapath % 'rotation_euler', 3,
                                                       self.sequences)
            ...
            bone.anim_scale = War3AnimationCurve.get(obj.animation_data, datapath % 'scale', 3,
                                                     self.sequences)  # get_curves(obj, datapath % 'scale', (0, 1, 2))
    ...
    def add_lights(self, billboard_lock, billboarded, obj, settings):
        ...
        if hasattr(obj.data, "mdl_light"):
            ...
            light.intensity_anim = War3AnimationCurve.get(obj.data.animation_data, 'mdl_light.intensity', 1,
                                                          self.sequences)  # get_curve(obj.data, ['mdl_light.intensity'])
            register_global_sequence(self.global_seqs, light.intensity_anim)

            light.atten_start = light_data.atten_start
            light.atten_start_anim = War3AnimationCurve.get(obj.data.animation_data, 'mdl_light.atten_start', 1,
                                                            self.sequences)  # get_curve(obj.data, ['mdl_light.atten_start'])
            register_global_sequence(self.global_seqs, light.atten_start_anim)

            light.atten_end = light_data.atten_end
            light.atten_end_anim = War3AnimationCurve.get(obj.data.animation_data, 'mdl_light.atten_end', 1,
                                                          self.sequences)  # get_curve(obj.data, ['mdl_light.atten_end'])
            register_global_sequence(self.global_seqs, light.atten_end_anim)

            light.color = light_data.color
            light.color_anim = War3AnimationCurve.get(obj.data.animation_data, 'mdl_light.color', 3,
                                                      self.sequences)  # get_curve(obj.data, ['mdl_light.color'])
            register_global_sequence(self.global_seqs, light.color_anim)

            light.amb_color = light_data.amb_color
            light.amb_color_anim = War3AnimationCurve.get(obj.data.animation_data, 'mdl_light.amb_color', 3,
                                                          self.sequences)  # get_curve(obj.data, ['mdl_light.amb_color'])
            register_global_sequence(self.global_seqs, light.amb_color_anim)

            light.amb_intensity = light_data.amb_intensity
            light.amb_intensity_anim = War3AnimationCurve.get(obj.data.animation_data, 'mdl_light.amb_intensity', 1,
                                                              self.sequences)  # get_curve(obj.data, ['obj.mdl_light.amb_intensity'])
